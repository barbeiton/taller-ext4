...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
    ...
    unsigned int block_size = 1024 << _superblock->log_block_size;
    unsigned short inode_size = _superblock->inode_size;

    unsigned int group = blockgroup_for_inode(inode_number);
    struct Ext2FSBlockGroupDescriptor *group_descriptor = block_group(group);
    unsigned int inode_table = group_descriptor->inode_table;

    unsigned int index = blockgroup_inode_index(inode_number);
    unsigned int block_offset = (index * inode_size) / block_size;

    unsigned char *block_buffer = new unsigned char[block_size];
    read_block(inode_table + block_offset, block_buffer);

    unsigned int inodes_per_block = block_size / inode_size;
    struct Ext2FSInode *inode_table_pointer = (struct Ext2FSInode *) block_buffer;
    struct Ext2FSInode *inode = new struct Ext2FSInode;
    inode = inode_table_pointer + (index - (inodes_per_block*block_offset));
...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
    ...
    unsigned int inodes_per_block = block_size/inode_size;
    struct Ext2FSInode *inode_table_pointer = (struct Ext2FSInode *) block_buffer;
    *inode = inode_table_pointer[index % inodes_per_block];
    //*inode = *((struct Ext2FSInode *) (block_buffer + (inode_size*(index % (block_size/inode_size)))));
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
    ...
    unsigned int block_size = 1024 << _superblock->log_block_size;
    unsigned int index_per_block = block_size / 4;
    unsigned int limit_level_one = 12 + index_per_block;
    unsigned int limit_level_two = limit_level_one + (index_per_block*index_per_block);

    unsigned char *block_buffer = new unsigned char[block_size];
    unsigned int *block_address_pointer = (unsigned int *) block_buffer;
    unsigned int block_pointer;
    if (block_number < limit_level_one) {
        read_block(inode->block[12], block_buffer);
        block_pointer =  block_address_pointer[block_number-12];
    }
    else if (block_number < limit_level_two) {
        unsigned int block_number_level = block_number - limit_level_one;
        unsigned int first_table_index = block_number_level / index_per_block;
        read_block(inode->block[13], block_buffer);
        unsigned int block_pointer_table = block_address_pointer[first_table_index];

        unsigned int index = block_number_level % index_per_block;
        read_block(block_pointer_table, block_buffer);
        block_pointer  = block_address_pointer[index];
    }
    else {
        std::cout << "Block number too high (ej 1)" << std::endl;
        delete[] block_buffer;
        return 0;
    }
...
static bool same_string(const char *filename, char *name, unsigned char name_length) {
    if (name_length == 0) {
        return false;
    }
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
    ...
    unsigned int block_size = 1024 << _superblock->log_block_size;
    unsigned int index_per_block = block_size / 4;
    unsigned int limit_level_two = 12 + index_per_block + (index_per_block*index_per_block);
    ...
    while ((!found) && block_number < limit_level_two) {
        unsigned char *next = block_buffer;
        read_block(get_block_address(from, block_number), block_buffer);

        unsigned int next_dir_entry = 0;
        while (next_dir_entry < block_size) {
            struct Ext2FSDirEntry *dir_entry = (struct  Ext2FSDirEntry*) next;
            if (same_string(filename, dir_entry->name, dir_entry->name_length)) {
                found = true;
                inode_number = dir_entry->inode;
                break;
            }
            next_dir_entry += dir_entry->record_length;
            next += dir_entry->record_length;
        }
        block_number++;
    }
    ...
    if (found) {
    	...
    	std::cout << "File inode not found: ";
	    std::cout << "get_file_inode_from_dir_inode" << std::endl;
        return NULL;
    }
}
